...
function(qt5_add_qml_module TARGET)
    set(options QMLPLUGIN_NO_GENERATE_TYPEINFO)
    ...
    if(QMLPLUGIN_NO_GENERATE_TYPEINFO)
    ...
    if(QMLPLUGIN_QML_FILES)
        ...
        if(__target_type MATCHES "SHARED_LIBRARY" AND QMLPLUGIN_GENERATE_TYPEINFO)
            set(__qml_plugin_qml_file_register "        Q_INIT_RESOURCE(${__qml_plugin_uri_name_lower});\n        //@uri ${__qml_plugin_uri_name}\n")
        else()
            set(__qml_plugin_qml_file_register "#ifdef ${__qml_plugin_uri_name_upper}_BUILD_STATIC_LIB\n        //@uri ${__qml_plugin_uri_name}\n")
        endif()
        string(APPEND __qml_plugin_qmldir_content "module ${TARGET}\n")
        if (__target_type MATCHES "SHARED_LIBRARY" AND QMLPLUGIN_GENERATE_TYPEINFO)
            ...
            string(APPEND __qml_plugin_qmldir_content "classname ${TARGET}Plugin\n")
        endif()
        string(APPEND __qml_plugin_qmldir_content "typeinfo ${QMLPLUGIN_TYPEINFO}\n")
        if (__target_type MATCHES "STATIC_LIBRARY")
            string(APPEND __qml_plugin_qmldir_content "prefer :${QMLPLUGIN_RESOURCE_PREFIX}${__qml_plugin_uri_dir}/\n")
        endif()
        foreach(qmlfile ${QMLPLUGIN_QML_FILES})
            ...
            string(APPEND __qml_plugin_qml_file_register "        qmlRegisterType(QUrl(\"qrc:${QMLPLUGIN_RESOURCE_PREFIX}${__qml_plugin_uri_dir}/${__qmlfile_full_name}\"), \"${__qml_plugin_uri_name}\", ${QMLPLUGIN_VERSION_MAJOR}, 0, \"${__qmlfile_name}\");\n")
        endforeach()
        if(NOT __target_type MATCHES "SHARED_LIBRARY" OR NOT QMLPLUGIN_GENERATE_TYPEINFO)
        ...
        if(QMLPLUGIN_DEPEND_MODULE AND __target_type MATCHES "SHARED_LIBRARY" AND QMLPLUGIN_GENERATE_TYPEINFO)
            set(__qml_plugin_qmldir_content "")
    ...
    if(QMLPLUGIN_RESOURCES)
        ...
        endforeach()
        configure_file(${__qml_plugin_current_dir}/project.qrc.in ${QMLPLUGIN_OUTPUT_DIRECTORY}/${__qml_plugin_target_name_lower}.qrc @ONLY)
        target_sources(${TARGET} PUBLIC ${QMLPLUGIN_OUTPUT_DIRECTORY}/${__qml_plugin_target_name_lower}.qrc)
    endif()

    ### Generate qmltypes
    if (__target_type MATCHES "SHARED_LIBRARY" AND QMLPLUGIN_GENERATE_TYPEINFO)
        set(__qmltypes_depend ${TARGET})
        if(QMLPLUGIN_DEPEND_MODULE AND QMLPLUGIN_GENERATE_TYPEINFO)
        ...
        add_custom_target(${TARGET}qmltypes ALL
            ...
            COMMAND ${QMLPLUGINDUMP_BIN} -nonrelocatable ${QMLPLUGIN_URI} ${QMLPLUGIN_VERSION_MAJOR}.0 ${QT_QML_INSTALL_DIR} > ${QMLPLUGIN_OUTPUT_DIRECTORY}/${QMLPLUGIN_TYPEINFO}
            COMMENT "Generating ${QMLPLUGIN_TYPEINFO}")
        foreach(depends ${QMLPLUGIN_DEPEND_MODULE})
            add_custom_target(${depends}clean ALL
                DEPENDS ${TARGET}qmltypes
                COMMAND ${CMAKE_COMMAND} -E remove_directory ${QT_QML_INSTALL_DIR}/${depends}
    ...
    if (__target_type MATCHES "LIBRARY")
        configure_file(${__qml_plugin_current_dir}/projectplugin.h.in ${CMAKE_CURRENT_BINARY_DIR}/${__qml_plugin_target_name_lower}plugin.h @ONLY)
        target_sources(${TARGET} PUBLIC ${CMAKE_CURRENT_BINARY_DIR}/${__qml_plugin_target_name_lower}plugin.h)
    endif()
endfunction()
